...
from .application_services import RSSApplicationService


def show(request: HttpRequest) -> HttpRequest:
    rss_application_service = RSSApplicationService()

    if request.method == "POST":
        feed_url: str = request.POST['feed_url']
        feed_items: List[Item] = rss_application_service.get_all_items(feed_url)
